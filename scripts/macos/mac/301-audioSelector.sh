...
# Filename: ~/github/dotfiles-latest/scripts/macos/mac/301-audioSelector.sh
# Lists available macOS output devices (speakers/headphones) and lets you select one via fzf
...
current_raw="$(SwitchAudioSource -t output -c 2>/dev/null || true)"
...
done < <(SwitchAudioSource -a -t output 2>/dev/null || true)
...
selected_line="$(
  printf "%s\n" "${menu_lines[@]}" |
    fzf --height=100% --reverse \
      --header="Select an output device (Esc to cancel)" \
      --prompt="Audio > " \
      --no-multi
)"
...
# Switch
SwitchAudioSource -t output -s "$selected_device" >/dev/null 2>&1

# echo "Selected output: $selected_device"
